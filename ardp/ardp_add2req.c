 ...
 *         ARDP_TOOLONG error.
 */
/* Rationale for using the type int to represent the buffer length:
   convenience in declarations; means that stuff won't break.
   Disadvantage if the buffer is too large for an int to represent its length.  Of course, if that occurs, we have more problems. */

enum ardp_errcode
ardp_add2req(RREQ	req,    /* Request to get the text */
	     int	flags,  /* Options to add2req      */
	     const char	*buf,   /* New text for request    */
	     int	buflen) /* Length of the new text  */
{
 ...
 keep_writing:
	...
	u_int32_t	taglen;	/* Length in net byte ord  */
	taglen = htonl((u_int32_t) buflen);
	memcpy4(pkt->ioptr, &taglen);
	pkt->ioptr += 4;
	remaining -= 4;
    }
