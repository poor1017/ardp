/* vqscanf.c
   ...
   9/97: Got rid of generality; gone to vqsscanf.  This means we duplicate
   code.  Ick. 
*/
...
/*
  ...
  !: If the '!' modifier is specified, the output buffer size follows.  If the
     ...
     integer constant GL_IO_BUFFER_OVERFLOW (guaranteed to be negative).  Note
     that 
  ...
  %r            char **     "Rest of the string".  Sets the pointer to point 
                  or        to the portion of the input string beyond this
                INPUT       point.  This is useful, for instance, to check
                void *      whether there was any leftover input in the string
                size_t      beyond a certain point.  Applying maximum
			    ...
			    This does the equivalent if qscanf() was called
			    directly from qscanf().  However, in this case, we
			    need the main argument to be an INPUT, not a char
			    **.  Also, we need two additional arguments so that
			    we have a place for the auxiliary storage buffer.

  %R            char **     "Rest of the string, skipping to the next line".
                or  INPUT   Equivalent to specifying 
                 void *     "%*( \t)%*[\r\n]%*( \t)%r" or "%~%*[\r\n]%~%r".
                 size_t     We can use this to go onto the next Prospero
...
#define match(cs, c)    ((c) != EOF && (am_quoting || is_in_charset(cs,(c))))

static int inpsetspn(charset cs, INPUT in);
static int qinpsetspn(charset cs, INPUT in, int am_quoting);
static int in_readcahead(INPUT in, int howfar);
...
/* This is not too efficient.  Good place to add efficiency.  */
/* On the other hand, it's not used that many times; it just does lookahead in
   case of parsing the Prospero quotation format; never used if you don't do
   quoting. */
static 
int
inline
in_readcahead(INPUT in, int howfar)
{
    ...
    if(howfar < 0)
	gl_function_arguments_error("in_readcahead(): howfar < 0");
    gl__input_copy(in, incpy, (void *) incpyaux, sizeof incpyaux);
    while (howfar-- > 0)
        in_incc(incpy);
...
inpsetspn(charset cs, INPUT oldin)
{
    ...
    gl__input_copy(oldin, in, &in_aux, sizeof in_aux);
    while (is_in_charset(cs, in_readc(in))) {
        ++retval;
        in_incc(in);
    }
    return retval;
...
qinpsetspn(charset cs, INPUT oldin, int am_quoting)
{
    ...
    for (; !in_eof(in); in_incc(in)) {
        ...
        case OUTSIDE_QUOTATION:
            if (in_readc(in) == '\'')
                state = IN_QUOTATION;
            else if (is_in_charset(cs, in_readc(in))) {
                ++count;
            } else  {
        ...
        case SEEN_POSSIBLE_CLOSING_QUOTE:
            if (in_readc(in) == '\'') {
                ...
                if (!is_in_charset(cs, in_readc(in)))
                    return count;
                ++count;
            }
            break;
...
{
    INPUT_ST    newin_st;
    int	    	newin_st_aux[10]; /* enough for ARDP/RREQ, only one we use. */
    int nmatches = 0;           /* no assignment-producing directives matched
                                   so far! */

    /* Copy 'in' so we don't consume the input. */
    gl__input_copy(in, &newin_st, &newin_st_aux, sizeof newin_st_aux);
    in = &newin_st;
...
with %r conversion.");
                if (!suppress) { /* suppression is really stupid in this case,
                    ...
                    if (in->flags & GL__IO_PERCENT_R_TARGET_IS_STRING) {
                        *va_arg(ap, const char **) = in->u.s.s;
                    } else {
                        INPUT inparg = va_arg(ap, INPUT);
			void *inpauxarg = va_arg(ap, void *);
			size_t inpauxargsiz = va_arg(ap, size_t);
                        gl__input_copy(in, inparg, inpauxarg, inpauxargsiz);
                    }
                    ++nmatches;
                }
                break;
...
don't know what you're doing.");
                ...
                if (quote && in_readc(in) == '\'' 
                    ...
                    /* quoted null string. */
		    in_incc(in), in_incc(in);/* skip over the two characters. */
                    if (!suppress) {
                        ++nmatches;
                        if (seen_ampersand)
                            *outp = stcopyr("", *outp);
...
don't know what you're doing.");
                ...
                if (quote && in_readc(in) == '\'' 
                    ...
                    /* quoted null string. */
		    in_incc(in), in_incc(in);/* skip quoted ''  */
                    if (!suppress) {
                        ++nmatches;
                        if (seen_ampersand)
                            *outp = stcopyr("", *outp);
                ...
                if (seen_ampersand) {
                    /* Set out to start of string; we can increment 'out', but
                       should leave '*outp' always at the start of the
		       string. */ 
